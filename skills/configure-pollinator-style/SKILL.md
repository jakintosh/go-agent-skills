...
description: Manage the marked Pollinator Style block in repository or global Codex guidance. Use to configure, update, inspect, repair, or remove ambient routing; default to repository scope unless the user explicitly requests global setup.
---
...
```sh
python3 <skill-dir>/scripts/configure_agents.py
```
...
- `--allow-override` only after the user confirms that an active `AGENTS.override.md` should be modified.

The script emits one JSON object. Treat it as the source of truth for the target path, whether the file and router already existed, and whether content changed. Do not manually recreate the router block.

If the script returns `confirmation_required`, explain that an active `AGENTS.override.md` takes precedence and ask whether to update it. If confirmed, rerun with `--allow-override`. If the script reports malformed or duplicate markers, stop without editing and report the exact file.
...
```text
Pollinator Style is ready for <repository name or “your Codex profile”>.
...
How it works: You do not need to mention Pollinator Style or name a skill in ordinary requests. Ask for the change normally. The router tells Codex to inspect the code, identify every affected engineering domain, select the matching installed Pollinator Style skills, and load only the references needed for the work.
...
When `changed` is true, use: `Start a new Codex task so the updated guidance is loaded from the beginning.`
