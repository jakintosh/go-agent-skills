...
- [Canonical Handler Flow](#canonical-handler-flow)
...
When verifying route wiring, middleware, request decoding, response DTOs, or status behavior, test the built router in-process; read [API testing](testing.md).

## Related Guides

- Read [error mapping](error-mapping.md) for service-to-HTTP error translation.
- Read [API testing](testing.md) for in-process API test shape.
- Read [API key integration](with-keys.md) when the API exposes key management or uses API-key auth.
- Read [CORS integration](with-cors.md) when the API needs runtime CORS origin management.
- Read [compose-go-http-routes](../../compose-go-http-routes/SKILL.md) for route-group composition.
- Read [work-with-go-servers](../../work-with-go-servers/SKILL.md) for production serving and deployment mounting.
- Read [work-with-go-services](../../work-with-go-services/SKILL.md) for domain service boundaries.
