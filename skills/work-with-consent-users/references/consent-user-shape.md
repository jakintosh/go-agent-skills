...
- Read [account-resolution.md](account-resolution.md) when implementing `internal/web` auth context, account lookup, profile refresh, protected routes, and CSRF handling.
- Read [integration-config.md](integration-config.md) when implementing production Consent client construction, integration manifest generation, handler mounting, public URL processing, and verification-key loading.
- Read [local-testing.md](local-testing.md) when wiring `pkg/testing.TestVerifier`, dev auth handlers, and tests that need authenticated requests without a Consent server.
- Read [production-deployment.md](production-deployment.md) when documenting or deploying the app with Consent registration, mounted verification keys, environment variables, reverse proxy paths, and operator checks.

## Common Touchpoints

- [`work-with-go-servers`](../../work-with-go-servers/SKILL.md) for serving package ownership and runtime composition
- [`work-with-go-config`](../../work-with-go-config/SKILL.md) for app-side config files, resolved runtime config, and secret paths
- [`work-with-go-services`](../../work-with-go-services/SKILL.md) for service boundaries and store contracts
- [`work-with-go-databases`](../../work-with-go-databases/SKILL.md) for database adapter shape and persistence guidance
- [`work-with-go-web-uis`](../../work-with-go-web-uis/SKILL.md) for server-rendered UI structure and mutation flow
- [`compose-go-http-routes`](../../compose-go-http-routes/SKILL.md) for mounting package-owned auth handlers
